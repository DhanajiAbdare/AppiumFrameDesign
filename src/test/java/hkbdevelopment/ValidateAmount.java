...
public class ValidateAmount extends BaseTest_General 
{
...
//			Double price = Double.parseDouble(amount.substring(1));			//index '0' represent $ sign & count from index '1'
			Double price = act.getFormattedAmount(amt_String); 				//call reusable method 
			totSum = totSum + price;
		}
		...
		Double display_tot_price = act.getFormattedAmount(tot_price);			//call reusable method 
		System.out.println("Total expected price of products :"+display_tot_price);
		Assert.assertEquals(totSum, display_tot_price);
		System.out.println("Both the price are same ie.products added to cart price & displayed price");
	}
}
